...
# -----------------------------
subscribers: Dict[str, Set[WebSocket]] = {}       # truck_id -> websockets
trip_subscribers: Dict[str, Set[WebSocket]] = {}  # trip_id  -> websockets
...
TruckStatus = Literal["available", "out_of_service", "booked"]
TripStatus = Literal["scheduled", "in_progress", "delivered", "cancelled"]
...
async def ensure_indexes():
    ...
    await db.trip_current_location.create_index([("trip_id", 1)], unique=True)
...
# -----------------------------
# Truck live location (kept for now)
# -----------------------------
@app.post("/api/trucks/{truck_id}/location")
async def update_truck_location(truck_id: str, payload: dict):
    lat = payload.get("lat")
    lng = payload.get("lng")
...
# -----------------------------
# Trip live location (NEW: option B)
# -----------------------------
@app.post("/api/trips/{trip_id}/location")
async def update_trip_location(trip_id: str, payload: dict):
    lat = payload.get("lat")
    lng = payload.get("lng")
    speed = payload.get("speed")
    ...
    await db.trip_current_location.update_one(
        {"trip_id": trip_id},
        {"$set": doc},
        upsert=True
    )

    if trip_id in trip_subscribers:
        dead = []
        for ws in list(trip_subscribers[trip_id]):
            try:
                await ws.send_json(doc)
            except Exception:
                dead.append(ws)
        for ws in dead:
            trip_subscribers[trip_id].discard(ws)

    return {"ok": True}

@app.get("/api/trips/{trip_id}/location")
async def get_trip_location(trip_id: str):
    doc = await db.trip_current_location.find_one({"trip_id": trip_id}, {"_id": 0})
    return doc or {"trip_id": trip_id, "lat": None, "lng": None, "ts": None}
...
async def get_trip(trip_id: str):
    ...
    booking = await db.bookings.find_one({"booking_id": trip["booking_id"]}, {"_id": 0})
...
async def ws_trip(websocket: WebSocket, trip_id: str):
    await websocket.accept()
    trip_subscribers.setdefault(trip_id, set()).add(websocket)

    last = await db.trip_current_location.find_one({"trip_id": trip_id}, {"_id": 0})
    if last:
        await websocket.send_json(last)
    ...
    try:
        ...
        trip_subscribers[trip_id].discard(websocket)
